$(document).on( 'pagebeforeshow','#category' ,function( e, data ) {
	...
	pageSelector = category;//urlObj.hash.replace( /\?.*$/, "" );
	...
	if ( data ) {
		...
		markup += ' <li><a  class="flokkur" id="'+sessionStorage.flokkur+'"haus="'+data.items[sessionStorage.flokkur].name+'" href="#offer"  data-transition="none"  ><h4>'+data.items[sessionStorage.flokkur].name+'</h4></a></li>';
		...
		$header.find( "h1" ).html( data.items[sessionStorage.flokkur].name );
		//console.log(markup);
		// Inject the category items markup into the content element.
		$("#category_list").html( markup );
